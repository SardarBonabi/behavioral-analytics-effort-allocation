...
The collection pipeline supports **public contribution share and cross-project participation**. It separates acquisition, validation, recovery, and database persistence so the analytical results are built on identifiable collection stages.
...
**Workflow:** discovery → profiles → weekly activity → participation → validated PostgreSQL records → analytical features.

Weekly activity retains the seven public contribution categories and the available aggregate private-contribution count. Activity-linked repository references become developer-project-week records. These references describe observed participation, not the full set of projects a developer could have chosen.
...
Validated public/private activity feeds [allocation features](allocation_features.py). Developer-project-week activity supports [project-entry modeling](project_entry.py); eligibility for the choice set is defined separately.
